...
function loadDevices(){
	...
	var devices = new Request.JSONP({
		url: 'http://www.ifixit.com/api/0.1/areas', 
		callbackKey: 'jsonp',
...
function parseResponse(data, indent){
	for (var property in data){
		if(property == 'DEVICES'){
			devicesFromArray(data[property], indent);
		}
		else{
			if(indent < 1){
				createItem(property, '')
			} else if(indent > 1){
				createSubItem(property, indent);
			}
			parseResponse(data[property], indent + 20);
		}
	}
}
function devicesFromArray(arr, indent){
	for(var index in arr){
		if (arr.hasOwnProperty(index)) {
				createDeviceItem(arr[index], indent);
		}
	}
}
...
function createSubItem(title, indent){
	// new div element
	var item = new Element('div');
	item.className = 'brand';
	//item.setStyle('background-image', 'url('+image+')');
	item.setStyle('background-size', '100%');
	item.setStyle('margin-left', indent+'px');
	item.innerHTML = '<span>'+title+'</span>';
	if(indent>10){
...
function createDeviceItem(title, indent){
	// new div element
	var item = new Element('div');
